...
describe("In Risk", function() {
  ...
  it("player 1 placing 30 unit on China from initial state (deploy phase(1)) is ilegal", function(){
    boardBefore = board;
    boardAfter = angular.copy(boardBefore);
    boardAfter.territory.China.owner = 1;
    boardAfter.territory.China.units = 30;
  ...
  it("player 1 placing 29 unit on China again after player 1 placing units on China (deploy phase(1)) before the board is full is illegal", function(){
    boardBefore = board;
  ...
  it("player1's last move in phase 1", function(){
    ...
    boardBefore.territory.Egypt.units = 8;
  ...
  it("changing from phase 1 to phase 2 \
     i.e. player 2 adding the last unit on Egypt(which belongs to player 2) after player1 has no remain units in phase 1\
     is legal", function(){
...
console.log("before move to Egypt is " + boardAfter.territory.Egypt.owner + " owned");
    boardAfter.territory.Egypt.units = 2;
  ...
  it("change phase from 2 to 3, i.e: player 1 adding the last remain unit on Japan(which belongs to player 1)\
    ...
    boardBefore.territory.China.units = 9;
    boardBefore.territory.Iceland.units = 9;
    boardBefore.territory.Irkutsk.units = 5;
    boardBefore.territory.India.units = 2;
    ...
    boardAfter.territory.Japan.units = 2;
    boardAfter.players.player1.remainUnits = 0;
    boardAfter.phase = 3;
    ...
    [{"setTurn": {"turnIndex" : 1}},
     {"set": {"key":"board", "value" : boardAfter}},
     {"set": {"key":"delta", "value" : "Japan"}}]);
    });

  it("change phase from 2 to 3, i.e: player 1 adding the last remain unit on Japan(which belongs to player 1)\
      but change the turnIndex to player2 is illegal", function(){
...
it("player 1 end turn after attacked Afghanistan(belongs to player 2, has 1 units) using China(belongs to player 1, has 9 units)\
   (attack phase(3)) is legal", function(){
    ...
    boardBefore.territory.Afghanistan.owner = 1;
    boardBefore.territory.Afghanistan.units = 3;
    boardBefore.territory.China.units = 6;

    boardBefore.players.player1.totalTerritories = 22;
    boardBefore.players.player2.totalTerritories = 20;
...
it("player 1 attack Great_Britain(belongs to player 1, has 1 units) from Iceland(belongs to player 2, has 9 units)\
 ...
 it("player 1 fortify Greenland(belongs to player 1, has 1 units) from China(belongs to player 1, has 9 units) \
  ...
  it("player2 reinforce in phase2 in Egypt ", function(){
    ...
    boardAfter = angular.copy(boardBefore);
    boardAfter.territory.Egypt.units = boardBefore.territory.Egypt.units + 1;
    boardAfter.players.player2.remainUnits = 6;
  ...
  it("change phase from 2 to 3, i.e: player 2 adding the last remain unit on Egypt(which belongs to player 2)\
      to change phase from 2 to phase 3 is legal", function(){
    ...
    boardBefore.territory.Iceland.units = 9;
    boardBefore.territory.Irkutsk.units = 8;
    boardBefore.territory.Egypt.units = 7;
    ...
    boardAfter.territory.Egypt.units = 8;
    boardAfter.players.player2.remainUnits = 0;
    boardAfter.phase = 3;
